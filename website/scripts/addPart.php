...
/*if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
	 if (empty($_POST["partName"])) {
	    $nameErr = "Part Name is required";
	  } else {
		  $name = test_input($_POST["partName"]);
	  }
	  $desc = test_input($_POST["desc"]);
	 if ($_POST["PartType"] == -1) {
	    $partTypeErr = "Part Type is required";
	  } else {
	  	$partType = $_POST["PartType"];
	  }
	  if(!errorsExist())
	  {
	  	//data valid - add to db
	  	if(!empty($_POST["imgFile"]))
	  	{
		  	$imgFile = $_POST["imgFile"];
		  }
	  	require 'dbAccess.php';
	  	$newPartId = addPart($name, $desc, $imgFile, $partType);
	  	if($newPartId != -1)
	  	{
		  	echo "Added Part " . $newPartId;
		  }
		  else
		  {
		  	echo "Failed to add Part";
		  }
	  }
}
...
{
	global $partTypeErr, $nameErr;
	return $partTypeErr == "" && $nameErr == "";
}
*/
?>
